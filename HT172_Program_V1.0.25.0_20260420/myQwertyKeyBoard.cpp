...
{
    if(fQwertyKey->bShow==true && fQwertyKey2->bKey2==false && fQwertyKey2->bShow==false)        //Steven 20150505 : 剛好在輸入時,Alarm會卡死
    {
    ...
    KeyCode      =iFunction;
    ChangeDecimalPoint();
    palQwertyKey ->Visible=!(iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_NUM_PAD);
    palNumKey    ->Visible=true;
    spbBackSpace2->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_NUM_PAD || iFunction&N_PORT || iFunction&N_IP_ADDR));
    spbClear2    ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_NUM_PAD || iFunction&N_PORT || iFunction&N_IP_ADDR));
    spbCancel2   ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_NUM_PAD || iFunction&N_PORT || iFunction&N_IP_ADDR));
    spbSummit2   ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_NUM_PAD || iFunction&N_PORT || iFunction&N_IP_ADDR));
    spbAdd1      ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbAdd10     ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbAdd100    ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbMinus1    ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbMinus10   ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbMinus100  ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbDP        ->Visible=((iFunction&N_IP_ADDR || iFunction&N_NUM_PAD || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD));
    spbPercent   ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    spbMinus     ->Visible=((iFunction&N_INTEGER || iFunction&N_DOUBLE) && !(iFunction&N_PASSWORD && iFunction&N_NUM_PAD));
    fQwertyKey   ->Top    =((iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_PORT || iFunction&N_IP_ADDR) && !(iFunction&N_PASSWORD))?200:400;
    fQwertyKey   ->Left   =((iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_PORT || iFunction&N_IP_ADDR) && !(iFunction&N_PASSWORD))?500:100;

    c=edQwertyContent->PasswordChar;
    if(iFunction&N_INTEGER || iFunction&N_DOUBLE || iFunction&N_NUM_PAD ||    //只有數字
...
//    TPanel    *PanelPtr  = dynamic_cast <TPanel *>   (Ptr);
    TCustomEdit     *EditPtr   = dynamic_cast <TCustomEdit *>   (Ptr);
      ...
      WORD &Key, TShiftState Shift)
{
    ...
    else if(Shift.Contains(ssCtrl))
    {
        if(Key=='C')
        {
            edQwertyContent->CopyToClipboard();
        }
        else if(Key=='V')
        {
            if(KeyCode&N_DOUBLE)
            {
                AnsiString clipboardText=Clipboard()->AsText;
                edQwertyContent->Text=atof(clipboardText.c_str());
            }
            else if(KeyCode&N_INTEGER)
            {
                AnsiString clipboardText=Clipboard()->AsText;
                edQwertyContent->Text=atoi(clipboardText.c_str());
            }
            else
            {
                edQwertyContent->PasteFromClipboard();
            }
        }
    }
}
      ...
      char &Key)
{
    ...
    else
    {
        ...
        {
            if((Key>='0' && Key<='9') || Key=='.' || (Key>='a' && Key<='z') || (Key>='A' && Key<='Z'))
                ;
            else
                Key=NULL;
        }
