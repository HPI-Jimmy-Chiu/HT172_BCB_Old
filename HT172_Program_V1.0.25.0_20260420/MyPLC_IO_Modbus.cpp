...
//ben 20230913 add
//==>
int iPLCStatusTask=-1;
bool bSafePLCThread=false;
HTimer hCheckThreadTime;
//<==
//ben 20230913 add
...
void InitPLCIO(AnsiString asIP,int iPort)
{
    ...
    for(i=0; i<(int)TMyPLC_IO_Modbus::PLC_IO_Members.size(); i++)
    {
...
void PLCStatusCheck()
{
    if(PlcComm.IsConnected()==false)
    {
        ...
        {
//            if(bFirstConnect==false)                                            //JerryYang 20250206 : fix
//            {
...
void PLCIOTaskCycle()
{
    ...
    switch(Task)
    {
        case 1:
            ...
            if(iTransactionID>0xffff)iTransactionID=0; // 0-0xffff (2byte)
            ...
            break;
        case 100:
            if(PlcComm.GetRecevie(iTransactionID,data))
            {
                //讀取成功
                //寫入Byte陣列
                ...
                for(i=0; i<INPUT_MAX_Slave; i++)
                {
                    for(j=0; j<INPUT_MAX_REGISTER; j++)
                    {
                        for(k=0; k<8; k++)
                        {
...
void TMyPLC_IO_Modbus::On()
{//Output模組 暫無使用 (預留)

}
void TMyPLC_IO_Modbus::Off()
{//Output模組 暫無使用 (預留)
...
void TMyPLC_IO_Modbus::IOByteOut(int port,byte Byte)
{//Output模組 暫無使用 (預留)
