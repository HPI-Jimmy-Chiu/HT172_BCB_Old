...
//AI(HT172-Maintainer) 20260314 : skip TQPF_Timer delays in simulation mode
#ifdef SOFT_SIMULATE
bool bSimSkipDelay = true;
#endif
...
{
#ifdef SOFT_SIMULATE
    if (bSimSkipDelay) return true;  //AI(HT172-Maintainer) 20260314 : instant timer expiry in sim
#endif
...
int TQPF_Timer::LatchCycleTimeSec(bool Start)
{
    ...
    else
    {
        ...
        QueryPerformanceFrequency(&rFreq);
        itmp=(rEndDelay.QuadPart-rStartDelay.QuadPart)/rFreq.QuadPart;  //Steven 20191016 : タ
    }
...
int TQPF_Timer::LatchCycleTime(bool Start)
{
    ...
    else
    {
        QueryPerformanceCounter(&rEndDelay); // 眔ヘ玡璸计
        QueryPerformanceFrequency(&rFreq);
        itmp=(rEndDelay.QuadPart-rStartDelay.QuadPart)*1000/rFreq.QuadPart;
    }
...
int TQPF_Timer::LatchCycleTimeUS(bool Start)     //Steven 20180808 (wei) : TTL丁虫эΘmicrosecond
{
    ...
    else
    {
        QueryPerformanceCounter(&rEndDelay); // 眔ヘ玡璸计
        QueryPerformanceFrequency(&rFreq);
        itmp=(rEndDelay.QuadPart-rStartDelay.QuadPart)*1000000/rFreq.QuadPart;
    }
