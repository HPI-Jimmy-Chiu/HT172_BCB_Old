...
void THGem::GetTimeInfo()
{
    ...
    if(iTimeFormat==1)                                                                                                                 //16 byte
        sprintf(GemClock,"%04d%02d%02d%02d%02d%02d%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec, SystemMSec/10);
    else if(iTimeFormat==2)                                                                                                            //14 byte
        sprintf(GemClock,"%04d%02d%02d%02d%02d%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
    else if(iTimeFormat==3)                                                                                                            //19 byte
        sprintf(GemClock,"%04d-%02d-%02dT%02d:%02d:%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
    else
        sprintf(GemClock,"%02d%02d%02d%02d%02d%02d", SystemYear%100, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
...
void __fastcall THGem::SaveSECSGEMErrToLog(AnsiString asSaveStr)                //20130409 Daver add
{
    ...
    asPath.sprintf("D:\\HT-172_Log\\SECS_GEM\\%s", tdSaveTime.FormatString("yyyy_mm_dd"));
    ForceDirectories(asPath);
    asFN.sprintf("%s\\SECSGEM_ErrLog.txt", asPath);
        ...
        : TForm(Owner)
{
    ...
    strcpy(DefaultAddress, "");                                                  //°²¦p²Ä¤@¦¸µn¤J,Address , Port ³£¤@¼Ë,®e©ö¨Ò¥~
    strcpy(DefaultPort, "");                                                     //¬G§Q¥Î "" ¨Ó¤£¨Ï¤¸¥ó enable
    strcpy(DefaultDeviceID, "");
    ...
    //pig 2014.08.27 KYEC start
    sgSECSECData->ColWidths[0]=40;
    ...
    RadioGroup1->OnClick=ManualCreateRadioGroup1Click;
    ...
    RecvMemoryBuffer=new TMemoryStream;  //test
    ProcBuffer=new TMemoryStream;  //test
    TempProcBuffer=new TMemoryStream;  //test
    iFileCount=0;
...
void __fastcall THGem::FormDestroy(TObject *Sender)
{
    ...
    try
    {
        ...
        ServerSocket1->Active=false;    //Steven 20140530
        ...
        delete RecvMemoryBuffer; //test
        delete ProcBuffer; //test
        delete TempProcBuffer; //test
...
AnsiString __fastcall THGem::ReadALED(AnsiString ALID)
{
    ...
    {
        S=SecsGemPath;
        ForceDirectories(S.c_str());
        S=SecsGemPath+"\\SPOOL";
        ForceDirectories(S.c_str());
        S=SecsGemPath+"\\SYSTEM";
        ForceDirectories(S.c_str());
        ALID_ALED=new TStringList;
        bALEDflag=false;
    }
    S=SecsGemPath+"\\SYSTEM";
    str.sprintf("%s\\ALID_ALED.ini", S.c_str());

    TIniFile *INIFile1 = new TIniFile(str);
    S=INIFile1->ReadString("ALID", ALID, "1");
    delete INIFile1;
    return S;
}
//------------------------------------------------------------------------------
void __fastcall THGem::WriteALED(AnsiString ALID,AnsiString S1)
{
    ...
    str.sprintf("%s\\ALID_ALED.ini", S.c_str());
    TIniFile *INIFile1 = new TIniFile(str);
    INIFile1->WriteString("ALID", ALID, S1);
    delete INIFile1;
}
//---------------------------------------------------------------------------
// V 1.0
...
//    unsigned int i;
    ...
    if(bConnect==false)
    {
        if(Local.SType==0 && Local.PType==0)
        {
            if(SCode==1 && FCode==13)     // Establish Communications Request (CR)
            {
            }
            else if(SCode==1 && FCode==14) //Establish Communications Request Acknowledge (CRA)
            {
            }
            else
            {
    ...
    else if(SCode==1 && FCode==14)
    {
    }
    else if(bSpooling==true)
    {
        WriteToSpoolFile();
        return;
    }
    */
    ...
    if(bUseClientSocket==true)
        ClientSocket1->Socket->SendBuf(LocalBuffer,LocalLength_4);
    else
    {
        if(ServerSocket1->Active==true && bServoSocketConnect==true)
        {
            if(ServerSocket1->Socket->Connections[0]->Connected)
                ServerSocket1->Socket->Connections[0]->SendBuf(LocalBuffer,LocalLength_4);
        }
    }
    ...
    ShowSFDescription(LocalBuffer[6]&0x7f,LocalBuffer[7]);
    ...
    */
    try                                                                         //20130204 Daver add
    {
        ...
        else
        {
            if(ServerSocket1->Active==true && bServoSocketConnect==true)
            {
                if(ServerSocket1->Socket->ActiveConnections>0)
                {
                    ...
                    {
                        /*
                        if(LocalLength_4 > 4096)                               //¥Î¨Ó¼ÒÀÀ¸ê®Æ¤Ó¤jºô¸ô·|¤À¤@¬q¤@¬q°e
                        {
                            TestP=LocalBuffer;
                            TestLen=LocalLength_4;
                            do
                            {
                                if(TestLen >= 4096)
                                {
                                    ServerSocket1->Socket->Connections[0]->SendBuf(TestP,4096);
                                    TestLen-=4096;
                                    MySleepEx(10, false);
                                    TestP+=4096;
                                }
                                ...
                                {
                                    ServerSocket1->Socket->Connections[0]->SendBuf(TestP,TestLen);
                                    TestLen=0;
                                    MySleepEx(10, false);
                                    TestP+=TestLen;
                                }
        ...
        if((FCode%2)==1 && Local.W_Bit==1)
        {
            ...
            //TimeLeft->Add(T3TimeOut*10);    //pig 2014.07.28 KYEC_SECS
            //<==
            // Joseph 20221108 (Jason) ·s¼W TCriticalSection E //
        }
    }
...
// V1.0
//---------------------------------------------------------------------------
void __fastcall THGem::ClientSocket1Connect(TObject *Sender,
...
// V1.0
//---------------------------------------------------------------------------
void __fastcall THGem::ClientSocket1Disconnect(TObject *Sender,
      TCustomWinSocket *Socket)
{
    GetTimeInfo();
...
// V1.0
//---------------------------------------------------------------------------
void __fastcall THGem::ClientSocket1Error(TObject *Sender,
...
// V1.0
//---------------------------------------------------------------------------
void __fastcall THGem::ClientSocket1Connecting(TObject *Sender,
      TCustomWinSocket *Socket)
{
        ...
        if(IsVCL==1 || IsVCL==2)
        {
            if(IsVCL==1)
            {
                ...
                else if(ComboBoxPtr!=NULL)
                {
                    ...
                    VCLIndex = ComboBoxPtr->ItemIndex;              // 2013.09.17 , Joye , KYEC SECS/GEM
                }
                else if(RadioGroupPtr!=NULL)
                {
                    if(RadioGroupPtr->ItemIndex>=0)
                    {
                        ...
                        VCLIndex = RadioGroupPtr->ItemIndex;        // 2013.09.17 , Joye , KYEC SECS/GEM
                    }
                    else
                        VCLStr="";
                }
...
bool THGem::DataItemOutSVNameList(AnsiString SVID)
{
    ...
    if(i>=0)
    {
        ...
        {
            DataItemOut(SVID.Length(), HType.ASCII_TYPE, SVID.c_str());
        }

        DataItemOut(SVName.Length(), HType.ASCII_TYPE, SVName.c_str());
        DataItemOut(SVUnit.Length(), HType.ASCII_TYPE, SVUnit.c_str());
        return true;
    }
...
void THGem::DataItemOutECNameList(AnsiString ECID)
{
    ...
    if(i>=0)
    {
        ...
        {
            DataItemOut(ECID.Length(), HType.ASCII_TYPE, ECID.c_str());
        }

        DataItemOut(strlen(ECName.c_str()), HType.ASCII_TYPE, ECName.c_str());
        ...
        {
            if(Type==HType.ASCII_TYPE) // 2013/05/08 lee
                DataItemOut(strlen(MinForAscii), Type, MinForAscii);     // 2013/05/08 lee
            else
        ...
        {
            if(Type==HType.ASCII_TYPE) // 2013/05/08 lee
                DataItemOut(strlen(MaxForAscii), Type, MaxForAscii);     // 2013/05/08 lee
            else
        ...
        {
            if(Type==HType.ASCII_TYPE)          // 2013/05/08 lee
                DataItemOut(strlen(DefaultForAscii), Type, DefaultForAscii);     // 2013/05/08 lee
            else
                DataItemOut(1, Type, PDefault); //
        }

        DataItemOut(strlen(ECUnit.c_str()), HType.ASCII_TYPE, ECUnit.c_str());
        return;
    }
    DataItemOut(6, HType.LIST_TYPE, NULL);
    j=atoi(ECID.c_str());
    ...
    if(bUseClientSocket==true)
    {
        switch(Task)
        {
            case 1:
                ...
                break;
            case 100:
                if(ClientSocket1->Address=="" || ClientSocket1->Port==0)
                    return false;
    ...
    else
    {
        switch(Task)
        {
            case 1:
                if(bOpenCommuncation==true)
                {
                    //ServerSocket1->Close();  // Â_½u«á³s¤£°_¨Ó 2013/07/18 lee §â Servo Socket close ¦A open ¬Ý¬Ý
                                               // ¤£¦æ·|·í !!!!!!!!!!!
            ...
            case 100:
                ...
                if(ServerSocket1->Port==0)
                    return false;

                //Eliot 2012_1108 start
            ...
            case 200:
                ...
                else if(ServerSocket1->Socket->ActiveConnections!=0)
                {
                    Task=1;
                    return true;
                }
...
void THGem::ReplyECDataChange()
{
    ...
    DataItemOut(3,HType.LIST_TYPE, NULL);
        DataItemOut(1,HType.INT_8_TYPE, &EVENT_SYSTEM_DATA_ID);
        DataItemOut(1,HType.INT_8_TYPE,&EVENT_SYSTEM_CEID_CHANGE_CONST);
        DataItemOut(1,HType.LIST_TYPE, NULL);
            DataItemOut(2,HType.LIST_TYPE, NULL);
                DataItemOut(1,HType.INT_8_TYPE, &EVENT_MACHINE_REPORTID);
                DataItemOut(0,HType.LIST_TYPE, NULL);
                //DataItemOut(1,HType.LIST_TYPE, NULL);
                //    DataItemOut(strlen("HT-3008"),HType.ASCII_TYPE,"HT-3008");
    StringOut("[Send EC Data Change]");
    SendLocalData();
    */
}
//------------------------------------------------------------------------------
...
int THGem::DoSpoolSendLocalData(unsigned char *Ptr)
{
    ...
    //Form2->Memo3->Lines->Add(RunLength);
    RunLength+=4;
    if(bUseClientSocket)
        ClientSocket1->Socket->SendBuf(Ptr,RunLength);
    else
        ServerSocket1->Socket->Connections[0]->SendBuf(Ptr, RunLength);
...
void THGem::DoSpool()
{
    ...
    if(bSpoolActive==false)
    {
        ...
        if(OldSpoolSystemMin!=SystemMin)
        {
            ...
            {
                S=AnsiString("del ")+SecsGemPath+AnsiString("\\spool\\*.*/q/f");
                system(S.c_str());
            }
    ...
    switch(Task)
    {
        case 1:
            ...
            S=SecsGemPath+AnsiString("\\spool\\")+FileListBox1->Items->Strings[0];
    ...
    switch(Task)
    {
        ...
        case 100:
            if(TraceDataResponseDelay[iIndex].TimerOff())
            {
                ...
                GetTimeInfo();
                if(iTimeFormat==1)                                                                                                                 //16 byte
                    sprintf(str, "%04d%02d%02d%02d%02d%02d%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec, SystemMSec/10);
                else if(iTimeFormat==2)                                                                                                            //14 byte
                    sprintf(str, "%04d%02d%02d%02d%02d%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
                else if(iTimeFormat==3)                                                                                                            //19 byte
                    sprintf(str, "%04d-%02d-%02dT%02d:%02d:%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
                else
                    sprintf(str, "%02d%02d%02d%02d%02d%02d", SystemYear%100, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
                DataItemOut(strlen(str), HType.ASCII_TYPE, str);
...
void THGem::DoUploadFileToHost_ForSingleFile()
{
    ...
    switch(Task)
    {
        ...
        case 100:
            ...
            iStoreCT=0;
            iTotalCount=TotalFileSize/iMaxSend;
        ...
        case 200:
            ...
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);      // 1.filename
...
void THGem::DoUploadFileToHost_ForMultiFile()
{
    ...
    switch(Task)
    {
        ...
        case 200:
            ...
            strcpy(str,ForMultiFileFileName.c_str());
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);      // 1.filename
            DataItemOut(1, HType.INT_4_TYPE, &iStoreCT);          // 2.iStoreCT
            DataItemOut(1, HType.INT_4_TYPE, &iTotalCount);       // 3.iTotalCount
...
//    FILE *P;
//    P=FilePUploadFileToHost_ForDirectoryFile;
    char Ptr[8000];
    AnsiString S;
...
//            if(P==NULL)
        ...
        case 200:
            ...
            strcpy(str, PathNameUploadFileToHost_ForDirectoryFile.c_str());
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);      // 1.Directory
            strcpy(str, FileNameUploadFileToHost_ForDirectoryFile.c_str());
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);      // 1.filename
...
//    int i;
    ...
    if(SECSConnectionState!=NULL)
    {
        if(bUseClientSocket==true)
        {
            if(flag1UpdateStatus || bClientSocketActive!=ClientSocket1->Active)
            {
                bClientSocketActive=ClientSocket1->Active;
        ...
        {
            if(iServoConnectCT!=ServerSocket1->Socket->ActiveConnections)
            {
                iServoConnectCT=ServerSocket1->Socket->ActiveConnections;
           ...
           GEMCommunicatingState->Caption == "1:OffLine"            )
        {
            ...
            if(SECSGEM_DoSeparate.Off())
            {
                ...
                try
                {
                    HGem->ServerSocket1->Close();
                    HGem->ClientSocket1->Close();
                }
                catch(...)
                {
...
//    FILE *P;
    ...
    str.sprintf("%s\\SYSTEM\\secs_gem.ini", SecsGemPath);
    TIniFile *INIFile1 = new TIniFile(str);
    if(EnableOrDisablePtr!=NULL)
    {
        i=INIFile1->ReadInteger("GEM", "EnableOrDisable", 0);
        if(i<0 || i>1)
            i=0;
        EnableOrDisablePtr->ItemIndex=i;
    }

//    if(OnLineOrOffLine!=NULL)
    {
        i=INIFile1->ReadInteger("GEM", "OnLineOrOffLine", 0);
        if(i<0 || i>1)
            i=0;
        OnLineOrOffLine->ItemIndex=i;
    }

//    if(RemoteOrLocal!=NULL)
    {
        i=INIFile1->ReadInteger("GEM", "RemoteOrLocal", 0);
        if(i<0 || i>1)
            i=0;
        RemoteOrLocal->ItemIndex=i;
    }

    i=INIFile1->ReadInteger("GEM", "ActiveOrPassive", 0);
    if(i==0)
    {
        bUseClientSocket=false;
    }
    ...
    edtIP->Text=INIFile1->ReadString("GEM", "Address", DefaultAddress);
    edtPort->Text=INIFile1->ReadString("GEM", "Port", DefaultPort);
    if(CUSTOMER_CODE==CC_KYEC_LEE)  //wei 20160309 SecsGem Port ±j¨î6000
        edtPort->Text=6000;
    edDeviceID->Text=INIFile1->ReadString("GEM", "DeviceID", DefaultDeviceID);
    //pig 2014.07.28 KYEC_SECS start
    edtT3TimeOut->Text=INIFile1->ReadString("GEM", "T3", T3TimeOut);
    edtT5TimeOut->Text=INIFile1->ReadString("GEM", "T5", T5TimeOut);
    edtT6TimeOut->Text=INIFile1->ReadString("GEM", "T6", T6TimeOut);
    edtT7TimeOut->Text=INIFile1->ReadString("GEM", "T7", T7TimeOut);
    edtT8TimeOut->Text=INIFile1->ReadString("GEM", "T8", T8TimeOut);
    ...
    if(bUseClientSocket==true)
    {
        ClientSocket1->Port=atoi(edtPort->Text.c_str());
        edtIP->Enabled=true;
    }
    else
    {
        ServerSocket1->Port=atoi(edtPort->Text.c_str());
        edtIP->Enabled=false;
    }

    //GemCheckBoxAcceptHostOnlineRequest
    bF=INIFile1->ReadBool("GEM","AcceptHostOnlineRequest",true);
    GemCheckBoxAcceptHostOnlineRequest->Checked=bF;

    GemCheckBoxShowBinary->Checked=false;   //Steven 20211109 : SECS¤£­n¨C¦¸³£Åã¥Übinary code

    bF=INIFile1->ReadBool("GEM", "ShowHeadInformation", false);
    GemCheckBoxShowHeadInformation->Checked=bF;

    bF=INIFile1->ReadBool("GEM","UseExtendedAlarm",false);
    GemCheckBoxUseExtendedAlarm->Checked=bF;

    bF=INIFile1->ReadBool("GEM","AnnotatedEventReport",false);
    chkAnnotatedEventReport->Checked=bF;

    bF=INIFile1->ReadBool("GEM","CheckMoreData",false);
    chkMoreMessageAbortProcess->Checked=bF;

    bF=INIFile1->ReadBool("GEM", "AddDefaultReport", true);          //2013/11/20  lee
    ckAddDefaultReport->Checked=bF;                               //2013/11/20  lee
...
//    {
//        bF=INIFile1->ReadBool("GEM", "ECChangeEventReport", false);
//        cbECChaneEventReport->Checked=bF;                               //2013/11/20  lee
//    }
//    else
//    {
...
//    bUseClientSocket=true;
    ...
    if(ckAddDefaultReport->Checked==false)                      //2013/11/20  lee
        ClearDefaultEvenReport();                               //2013/11/20  lee
...
void __fastcall THGem::SaveSystemDefault()
{
    ...
    AnsiString S;
    S=SecsGemPath+"\\SYSTEM";
    sprintf(str, "%s\\secs_gem.ini", S.c_str());
    TIniFile *INIFile1=new TIniFile(str);

    if(EnableOrDisablePtr!=NULL)
    {
        i=EnableOrDisablePtr->ItemIndex;
        INIFile1->WriteInteger("GEM", "EnableOrDisable", i);
    }

//    if(OnLineOrOffLine!=NULL)
    {
        i=OnLineOrOffLine->ItemIndex;
        INIFile1->WriteInteger("GEM", "OnLineOrOffLine", i);
    }

//    if(RemoteOrLocal!=NULL)
    {
        i=RemoteOrLocal->ItemIndex;
        INIFile1->WriteInteger("GEM", "RemoteOrLocal", i);
    }

    i=RadioGroup1->ItemIndex;
    INIFile1->WriteInteger("GEM", "ActiveOrPassive", i);

    INIFile1->WriteString("GEM", "Address",edtIP->Text.c_str());
    INIFile1->WriteString("GEM", "Port",edtPort->Text.c_str());
    INIFile1->WriteString("GEM", "DeviceID",edDeviceID->Text.c_str());
    //pig 2014.07.28 KYEC_SECS start
    INIFile1->WriteString("GEM", "T3",edtT3TimeOut->Text.c_str());
    INIFile1->WriteString("GEM", "T5",edtT5TimeOut->Text.c_str());
    INIFile1->WriteString("GEM", "T6",edtT6TimeOut->Text.c_str());
    INIFile1->WriteString("GEM", "T7",edtT7TimeOut->Text.c_str());
    INIFile1->WriteString("GEM", "T8",edtT8TimeOut->Text.c_str());
    //pig 2014.07.28 KYEC_SECS end

    INIFile1->WriteBool("GEM", "AcceptHostOnlineRequest", GemCheckBoxAcceptHostOnlineRequest->Checked);
    INIFile1->WriteBool("GEM", "ShowBinaryData", GemCheckBoxShowBinary->Checked);
    INIFile1->WriteBool("GEM", "ShowHeadInformation", GemCheckBoxShowHeadInformation->Checked);
    INIFile1->WriteBool("GEM", "UseExtendedAlarm", GemCheckBoxUseExtendedAlarm->Checked);
    INIFile1->WriteBool("GEM", "AnnotatedEventReport", chkAnnotatedEventReport->Checked);
    INIFile1->WriteBool("GEM", "CheckMoreData", chkMoreMessageAbortProcess->Checked);
    INIFile1->WriteBool("GEM", "AddDefaultReport", ckAddDefaultReport->Checked);   //2013/11/20  lee
    INIFile1->WriteBool("GEM", "ECChangeEventReport", cbECChaneEventReport->Checked);  //JerryYang 20200520 ÂÂ¹q¸£°µDoReportECDataChangeCheck¨ç¦¡·|¼vÅTUPH,§ï¦¨¥\¯à¿ï¶µ
...
//    int i,j;//,ct=0;
    ...
    {
        //HGem[i]->DB->Clear();
        //for(j=0 ; j< HGem[i]->WaitShowString->Count ; j++)
        //    HGem[i]->DB->Lines->Add(HGem[i]->WaitShowString->Strings[j]);
...
void __fastcall THGem::Timer1Timer(TObject *Sender)
{
    ...
    bTimerRunning=true;
    //Ifor 20170425 add Ãö³¬ SECS GEM Function »Ý¥ý¤UÂ_½u©R¥O
    //==>
    if(USE_SECS_GEM>0)
    {
    ...
    //Ifor 20180913 (Steven) : add SECS GEM Â_½u«á»Ý©µ¿ð¶}±ÒÁ×§K³y¦¨µLªk³s½u°ÝÃD
    //==>
    if(bSetDoSeparateWait && bSECSGEM_DoSeparate)
    {
...
void THGem::TerminalRequest(AnsiString S)
{
    ...
    DataItemOut(S.Length(), HType.ASCII_TYPE, S.c_str());
    SendLocalData();
...
void THGem::SetSpoolActive(bool Active)
{
    if(Active==false)
    {
        ...
        if(FileListBox1->Items->Count!=0)
        {
            int ret=ShowMyMessageBox_YES_NO("All spool data will be clear,Sure to clear ?");
            if(ret==TMyMessageBox::msgrtnYES)
            {
                AnsiString S=AnsiString("del ")+SecsGemPath+AnsiString("\\spool\\*.*/q/f");
                system(S.c_str());
            }
            else
            {
...
void __fastcall THGem::ReadAlamData()
{
    ...
    Filename=SecsGemPath+"\\SYSTEM\\AlarmData.def";
    ...
    //Ifor 20170523 (wei) add Á×§KAlarmData.def¸ê®ÆªÅ¥Õ³y¦¨SECS GEM µLªk¥¿±`±Ò°Ê
    //==>
    try
    {
        while(1)
...
void __fastcall THGem::WriteAlamData()
{
    ...
    Filename=SecsGemPath+"\\SYSTEM";
    ForceDirectories(Filename);                                                 //2013/07/25 Lee
    Filename=SecsGemPath+"\\SYSTEM\\AlarmData.def";
...
void __fastcall THGem::ReportAlarm(AnsiString AlarmCode, bool bIsJam, int iDuplicateError, AnsiString SubMessage, bool bReleaseAlm)     //JerryYang 20170504 (Steven) bReleaseAlm¬°trueªí¥Ü¸Ñ°£alarm»Ý¤W³ø0x00
{
    ...
    for(i=1; i<strGrdAlarm->RowCount; i++)
    {
        if(strGrdAlarm->Cells[2][i]==AlarmCode)
        {
            if(strGrdAlarm->Cells[7][i]=="1")
            {
                ...
                if(CUSTOMER_CODE==CC_KYEC_JCTHIU || CUSTOMER_CODE==CC_KYEC_CHEN || CUSTOMER_CODE==CC_KYEC_LEE || CUSTOMER_CODE==CC_DL_TEK)   //JerryYang 20170317 (wei) ¥u¦³¨Ê¤¸ªºAlarm Report¤~¦^¶Ç6¶µ
                {
                    DataItemOut(6, HType.LIST_TYPE, NULL);
                    DataItemOut(strlen(str), HType.ASCII_TYPE, str);                                //®É¶¡
                    ...
                    DataItemOut(strGrdAlarm->Cells[6][i].Length(), HType.ASCII_TYPE, strGrdAlarm->Cells[6][i].c_str()); //Alarm Text
                    DataItemOut(strGrdAlarm->Cells[10][i].Length(), HType.ASCII_TYPE, strGrdAlarm->Cells[10][i].c_str()); //Alarm Position

                    aMessType+=(bIsJam)?"1,":"0,";
                    aMessType+=AnsiString(iDuplicateError)+",";
                    aMessType+=SubMessage;
                    DataItemOut(aMessType.Length(), HType.ASCII_TYPE, aMessType.c_str());           //Alarm Sub Message
                }
...
//                    else
                    ...
                    DataItemOut(aMessType.Length(), HType.ASCII_TYPE, aMessType.c_str()); //ALTX Alarm Sub Message
                }
                SendLocalData();
            }
            return;
...
void __fastcall THGem::ReportAlarmWithMessage(AnsiString ALID, AnsiString ALTX, AnsiString sHappenedTime, AnsiString sProcessKey, bool SameCase)
{
    ...
    else
    {
        ...
        if(GemCheckBoxUseExtendedAlarm->Checked)
        {
            ...
            DataItemOut(5, HType.LIST_TYPE, NULL);
            DataItemOut(8, HType.ASCII_TYPE, ALID.c_str());
            DataItemOut(40, HType.ASCII_TYPE, ALTX.c_str());
            DataItemOut(20, HType.ASCII_TYPE, sHappenedTime.c_str());
            DataItemOut(10, HType.ASCII_TYPE, sProcessKey.c_str());
        ...
        else
        {
            ...
            strcpy(str,ALID.c_str());
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);
            strcpy(str,ALTX.c_str());
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);
            SendLocalData();
        }
    }
...
int THGem::SetReceipeDirectoryAndGlobalName(AnsiString Path, AnsiString FileMask, int Type)
{
    ...
    else
    {
        ...
        {
            S=FileListBox2->Items->Strings[i];
            S=S.SubString(1,S.Length()-4);   //JerryYang 20241015 : ÀÉ¦W¥h±¼.ini
            UploadFileString->Add(S);
        }
        return FileListBox2->Items->Count;
    }
...
{
    strcpy(GemMDLN,    Mdln.c_str());
    strcpy(GemSOFTREV, SoftVer.c_str());
}
//==============================================================================
// V 1.0
//==============================================================================
...
void __fastcall THGem::GemDownLoadRemoteFileNameClick(TObject *Sender)
{
    ...
    AnsiString S;
    char str[256];
    for(i=0; i<GemRemoteReceipeList->Items->Count; i++)
...
void __fastcall THGem::DoDownLoadRemoteFile()
{
    ...
    switch(Task)
    {
        ...
        case 100:
            InitLocalHead(7, 5, 1);
            OldFile=RequestRemoteDownLoad->Strings[0];
            strcpy(str, RequestRemoteDownLoad->Strings[0].c_str());
            DataItemOut(strlen(str), HType.ASCII_TYPE, str);
            bReceiveS7F6=false;
...
// V1.1
//---------------------------------------------------------------------------
void __fastcall THGem::ServerSocket1ClientConnect(TObject *Sender,
      TCustomWinSocket *Socket)
{
    AnsiString S, S0, S1, S2, S3;
    for(int i=0; i<ServerSocket1->Socket->ActiveConnections; i++)
    {
        S="Connect "+AnsiString(i)+":" +ServerSocket1->Socket->Connections[i]->LocalAddress;
        StringOut(S);
    }
...
// V1.1
//---------------------------------------------------------------------------
void __fastcall THGem::ServerSocket1ClientError(TObject *Sender,
      TCustomWinSocket *Socket, TErrorEvent ErrorEvent, int &ErrorCode)
{
    //20130308 Daver add
    AnsiString S;
...
// V1.1
//---------------------------------------------------------------------------
void __fastcall THGem::ServerSocket1ClientDisconnect(TObject *Sender,
      TCustomWinSocket *Socket)
{
    bServoSocketConnect=false;
    GetTimeInfo();
...
// V1.1
//---------------------------------------------------------------------------
void __fastcall THGem::ManualCreateRadioGroup1Click(TObject *Sender)
{
    if(bShow==true)
    {
...
void __fastcall THGem::SaveTCPIPRecieveData(unsigned char *DataBuf, int iLen)
{
    ...
    asPath.sprintf("%s\\logs\\SECS_GEM\\%s", SecsGemPath.c_str(),tdSaveTime.FormatString("yyyy_mm_dd").c_str());
    ForceDirectories(asPath);
...
{
    strcpy(DefaultAddress, Address);
    strcpy(DefaultPort, Port);
...
{
    int ret, i, j, len, iTotalItem;//,iHCACK=0;
    unsigned char Type;
...
void __fastcall THGem::SetCEIDContent(unsigned iCeid, AnsiString CeidAlias, unsigned iReportCount, unsigned *iReportIDData, int Mode) // mode=1 ¬OHandler ,=0 ¬O Host ©w¸q¦a
{
    ...
    unsigned iBackID[1024], ct=0;
    AnsiString S;

    for(y=1; y<strGrdCEID->RowCount; y++)
    {
    ...
    if(bSearchOK==false)
    {
        for(y=1; y<strGrdCEID->RowCount; y++)
        {
            if(strGrdCEID->Cells[0][y]=="")
            {
                ...
                for(i=3; i<strGrdCEID->ColCount; i++)
                    strGrdCEID->Cells[i][y]="";
                for(x=3; x<(iReportCount+3); x++)
...
unsigned __fastcall THGem::GetCEIDContent(unsigned iCeid, unsigned *iReportIDData, int Mode)
{
    ...
    for(y=1; y<strGrdCEID->RowCount; y++)
    {
        if(strGrdCEID->Cells[0][y]==iCeid)
        {
            for(x=3; x<strGrdCEID->RowCount; x++)
            {
    ...
    if(bSearchOK==false)
    {
        for(y=1; y<stdGridReportID->RowCount; y++)
        {
            if(stdGridReportID->Cells[0][y]=="")
            {
                ...
                for(x=2; x<stdGridReportID->ColCount; x++)
                    stdGridReportID->Cells[x][y]="";
                for(i=2; i<(iReportCount+2); i++)
...
bool __fastcall THGem::DeleteReportID(unsigned iCeid, int Type)                  //2013/11/20 lee
{
    ...
    for(y=1; y<stdGridReportID->RowCount; y++)
    {
        if(stdGridReportID->Cells[0][y]==iCeid && stdGridReportID->Cells[1][y]==Type)  //2013/11/20 lee
        {
            pos=y;
            bSearchOK=true;
            break;
    ...
    if(bSearchOK==true)
    {
        for(y=pos; y<(stdGridReportID->RowCount-1); y++)
        {
            for(x=0; x<stdGridReportID->ColCount; x++)
                stdGridReportID->Cells[x][y]=stdGridReportID->Cells[x][y+1];
        }
        y=stdGridReportID->RowCount-1;
        for(x=0; x<stdGridReportID->ColCount; x++)
            stdGridReportID->Cells[x][y]="";
        return true;
    }
...
bool __fastcall THGem::DeleteReportIDOfCeid(unsigned iCeid)
{
    ...
    for(y=1; y<strGrdCEID->RowCount; y++)
    {
        for(x=3; x<strGrdCEID->ColCount; x++)
        {
...
unsigned __fastcall THGem::GetReportIDContent(unsigned iCeid, unsigned *iReportIDData, int Mode)
{
    ...
    for(y=1; y<stdGridReportID->RowCount; y++)
    {
        if(stdGridReportID->Cells[0][y]==iCeid)
        {
            for(x=2; x<stdGridReportID->RowCount; x++)
            {
...
void __fastcall THGem::SendRepoerID(unsigned iReportID)
{
    ...
    DataItemOut(ct, HType.LIST_TYPE, NULL);    // L,a
...
void __fastcall THGem::SendAnnotatedRepoerID(unsigned iReportID)
{
    ...
    for(unsigned i=0; i<ct; i++)
    {
        DataItemOut(2, HType.LIST_TYPE, NULL);    // L,a
        svid=iSVIDData[i];
...
void __fastcall THGem::EventReport(unsigned iDataID, unsigned iCeid)
{
    ...
    //==>
    if(iCeid==SECS_EVENT.PressExit)    //Exit Pressed (SECS_EVENT.PressExit)
    {
        DoSeparate();
        try
        {
...
//------------------------------------------------------------------------------
bool THGem::SplitStrByTabOnly(AnsiString &source, AnsiString &result)
{
    if (source.IsEmpty())
    {
        result = "";
    ...
    int start = 1;
    while (start <= source.Length() && (source[start] == '\t' || source[start] == '\r' || source[start] == '\n'))
        start++;
    ...
    int tabPos = source.Pos("\t");
    if (tabPos == 0)
    {
        result = source.SubString(start, source.Length());
        source = "";
    }
    else
    {
        result = source.SubString(start, tabPos - start);
        source = source.SubString(tabPos + 1, source.Length());
    }
    result=CustomTrimRightCRLF(result);
    return true;
}
//------------------------------------------------------------------------------
void __fastcall THGem::PasteStringGridAsTabFormat(TStringGrid *strGrd, TStringList *memoPtr)
{
    ...
    for(y = 0; y < memoPtr->Count; y++)
    {
        if(y >= strGrd->RowCount)
            break;
        str = memoPtr->Strings[y];
        x = 0;
        do
        {
            flag=SplitStrByTabOnly(str, str2);
            if(flag == false)
                break;
            StartX = x;
            StartY = y;
...
//------------------------------------------------------------------------------
AnsiString THGem::CustomTrimRightCRLF(const AnsiString &str)
{
    AnsiString result = str;
    int len = result.Length();
    while (len > 0 && (result[len] == '\r' || result[len] == '\n'))
        len--;
...
void __fastcall THGem::ReadEventReportData()
{
    ...
    memoPtr=new TStringList;
    Filename=SecsGemPath+"\\SYSTEM\\EventReport_CEID.def";
    ...
    Filename=SecsGemPath+"\\SYSTEM\\EventReport_ReportID.def";
    if(CheckFileExist(Filename.c_str())==false)
    {
        delete memoPtr;                                                         //16.09.14.00 Roy Add
        return;
    ...
    else
    {
        ...
        else
        {
            if((CUSTOMER_CODE==CC_TFME_CHINA && GemControlState>1) || CUSTOMER_CODE!=CC_TFME_CHINA)  //JerryYang 20200527 Onlineª¬ºA¤~±µ¦¬³s½u¥H¥~S,F code
            {
                ...
                else if(HGem->Remote.MessageID_S==14 && HGem->Remote.MessageID_F==2)    // [S14F1] Get Attr Request
                    HSys.MyGem->ProcessS14F2_GetAttrData();
...
//==============================================================================
void __fastcall THGem::ClientSocket1Read(TObject *Sender, TCustomWinSocket *Socket)
{
...
void __fastcall THGem::ProcessSocketReceiveData()
{
    ...
    if(ServerSocket1->Socket->ActiveConnections>1)        //2013/09/30  lee
    {
        bInProcess=false;
...
bool __fastcall THGem::ReadECEnableData()
{
    ...
    memoPtr=new TStringList;
    Filename=CurrentDirectory+"\\SYSTEM\\ECEnableData.def";
    if(FileExists(Filename)==false)
    {
        delete memoPtr;                                                         //16.09.14.00 Roy Add
        return false;
...
void __fastcall THGem::WriteECEnableData()
{
    ...
    Filename=CurrentDirectory+"\\SYSTEM";
