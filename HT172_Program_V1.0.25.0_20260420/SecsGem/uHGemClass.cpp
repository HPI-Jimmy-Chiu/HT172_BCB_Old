...
void HTGem::S1F2_OnLineData()
{
    ...
    HGemPtr->DataItemOut(strlen(HGemPtr->GemMDLN),    HType.ASCII_TYPE, HGemPtr->GemMDLN);
    HGemPtr->DataItemOut(strlen(HGemPtr->GemSOFTREV), HType.ASCII_TYPE, HGemPtr->GemSOFTREV);
    HGemPtr->SendLocalData();
}
//---------------------------------------------------------------------------
// V 1.0
...
//    void *P;
    ...
    if(HGemPtr->GetDataItemLenAndTypeAndDelete(SVlen, Type)==1)
    {
        if(Type==HType.LIST_TYPE || SVlen==0)
        {
            ...
            {
                for(i=1; i<HGemPtr->strGrdCEID->RowCount; i++)
                {
                ...
                for(j=0; j<len; j++)
                {
                    ...
                    strncpy(ceidName, HGemPtr->strGrdCEID->Cells[2][pos].c_str(), sizeof(ceidName));
                    HGemPtr->DataItemOut(1, HType.UINT_4_TYPE, &uint4SV);
                    HGemPtr->DataItemOut(strlen(ceidName), HType.ASCII_TYPE, ceidName);
                    SVIDList->Clear();
                    for(k=3; k<HGemPtr->strGrdCEID->ColCount; k++)
                    {
            ...
            else
            {
                ...
                for(j=0; j<len; j++)
                {
                    ...
                    if(pos==0)  // no such CEID
                    {
                        ...
                        strncpy(ceidName, "", sizeof(ceidName));
                        HGemPtr->DataItemOut( 1, HType.UINT_4_TYPE,&uint4SV);
                        HGemPtr->DataItemOut( strlen(ceidName), HType.ASCII_TYPE,ceidName);
...
void HTGem::S2F18_DateandTimeData()
{
    ...
    HGemPtr->DataItemOut(strlen(HGemPtr->GemClock), HType.ASCII_TYPE, HGemPtr->GemClock);
    HGemPtr->SendLocalData();
}
// ---------------------------------------------------------------------------
// V 1.0
...
int HTGem::S2F24_TraceInitializeAcknowledgeSub()
{
    ...
    unsigned int i;
    unsigned char Type;
    char str[256],dsper[64];//,SVID_Str[256];
...
{
    int i,len,j,svlen,ret,temp;//,svid;
    unsigned char Type;
//    char str[256];
    AnsiString DATAID,ReportID,sSVID;
    TStringList *P;

    for(i=0; i<HGemPtr->slTempReportID->Count; i++)                                        // 若上一次的資料尚未清除則要清除
    {
...
void HTGem::S2F34_ProcessHostSendReportID()
{
    ...
    // 檢查是否已經定義了
    for(i=0; i<HGemPtr->slTempReportID->Count; i++)
    {
    ...
    for(i=0; i<HGemPtr->slTempReportID->Count; i++)
    {
        ...
        if(strPtr->Count==0)
        {
            //DeleteReportID( atoi(ReportID.c_str()));                            // 把 report id 表內地 report id 全部 delete
...
//    char str[256];
    ...
    for(i=0; i<HGemPtr->slTempCeID->Count; i++)     // 若上一次遞資料尚未清除記憶體則要清除
    {
...
int HTGem::S2F42_Host_Command_Acknowledge()
{
    ...
    if(HGemPtr->DataItemIn(2, HType.LIST_TYPE, NULL)==1)                        // 需要補充多重 Command
    {
        ...
        {
            FSECS->GemBtnOnlineRemote->Click();
            HCACK=0;
        }
        else if(S=="ONLINE_LOCAL")
        {
            FSECS->GemBtnOnlineLocal->Click();
            HCACK=0;
        }
        else if(S=="PP_SELECT")
        {
             if(HGem->DataItemIn(2, HType.LIST_TYPE, NULL)==1)
             {
                 ...
                 if(ret==-1)
                    S1="";
                 S1=CommandStr;
                 fMain->cb_WorkFile->Text=S1;

                 for(int i=0; i<fMain->cb_WorkFile->Items->Count; i++)
                 {
                    if(fMain->cb_WorkFile->Items->Strings[i]==S1)
                    {
                        fMain->cb_WorkFile->ItemIndex=i;
//                        fMain->cb_WorkFile-
                        break;
                    }
                 }
        ...
        {
            HGemPtr->StringOut("S2,F41 Remote command not exist error ");
            HCACK=1;
        }
        HGemPtr->InitLocalHead(2, 42, 0);
...
{
//    char str1[256],str2[256],str[1024];
//    unsigned P,ALID4;
//    unsigned short ALID2;
    ...
    if(HGemPtr->GetDataItemLenAndType(SVlen,Type)==1)
    {
                    ...
                    for(i=0; i<SVlen; i++)
                    {
                        ...
                        if(j>=0)
                        {
                            ...
                            if(Type==HType.UINT_8_TYPE)  //Steven 20140911 : 修正INT_8_TYPE & UINT_8_TYPE
                            {
                                ...
                                HGemPtr->DataItemOut(1,HType.UINT_8_TYPE,&uint8SV);     // ID
                            }
                            else if(Type==HType.UINT_4_TYPE)
                            {
                            ...
                            else if(Type==HType.UINT_2_TYPE)
                            {
                                ...
                                HGemPtr->DataItemOut(1,HType.UINT_2_TYPE,&uint2SV);     // ID
                            }
                            else if(Type==HType.INT_8_TYPE)  //Steven 20140911 : 修正INT_8_TYPE & UINT_8_TYPE
                            {
                                int8SV=_atoi64(HGemPtr->strGrdAlarm->Cells[1][j].c_str());
                                HGemPtr->DataItemOut(1,HType.INT_8_TYPE,&int8SV);     // ID
                            }
                            else if(Type==HType.INT_4_TYPE)
                            {
                                int4SV=atoi(HGemPtr->strGrdAlarm->Cells[1][j].c_str());
                                HGemPtr->DataItemOut(1,HType.INT_4_TYPE,&int4SV);     // ID
                            }
                            else if(Type==HType.INT_2_TYPE)
                            {
                                int2SV=atoi(HGemPtr->strGrdAlarm->Cells[1][j].c_str());
                                HGemPtr->DataItemOut(1,HType.INT_2_TYPE,&int2SV);     // ID
                            }
                            str=HGemPtr->strGrdAlarm->Cells[4][j];           // message
        ...
        else
        {
            ...
            for(i=1; i<HGemPtr->strGrdAlarm->RowCount; i++)
            {
                ...
                HGemPtr->DataItemOut(1,HType.UINT_4_TYPE,&uint4SV);     // ID
                str=HGemPtr->strGrdAlarm->Cells[4][i];           // message
                HGemPtr->DataItemOut(str.Length(),HType.ASCII_TYPE,str.c_str());
            }
...
//    char str[256];
    ...
    if(HGemPtr->DataItemIn(1, HType.UINT_1_TYPE, &RSDC)==1)
    {
        if(RSDC==1)
        {
            S=AnsiString("del ")+HGemPtr->CurrentDirectory+AnsiString("\\spool\\*.*/q/f");
            system(S.c_str());
        }
        HGemPtr->LocalAcknowledge(6, 24, 0);
    }
...
void __fastcall HTGem::S7F18_DeleteProcessProgramAcknowledge()
{
    ...
    */
    ...
    int len,i,ret,slen;
    unsigned char Type;
    AnsiString S,S1,asLastFileName;
    asLastFileName=GetLastOpenFN();     //JerryYang 20170626 (Steven) 使用中的工作檔不能被刪除
...
void HTGem::S7F20_CurrentEPPDData()
{
    int i,count;
    char str[256];
    if(HGemPtr->SV_70_UNT1_ReceipeStruct!=0)     //Steven 20140902 : 0 --> 2
    {
    ...
    HGemPtr->SetReceipeDirectoryAndGlobalName(DataPath, "*.ini", 0); //  0 : 只傳副檔名為 bld  //Steven 20140902 : 0 --> 2  //Steven 20150203 : 傳送之前要先重新整理
    ...
    for(i=0; i<count; i++)
    {
        strncpy(str, HGemPtr->UploadFileString->Strings[i].c_str(), sizeof(str));
        HGemPtr->DataItemOut(strlen(str),HType.ASCII_TYPE,str);
    }
    HGemPtr->SendLocalData();
}
//-----------------------------------------------------------------------------
...
void HTGem::S9F7_IllegalData(AnsiString S)
{
    ...
    HGemPtr->DataItemOut(S.Length(),HType.ASCII_TYPE, S.c_str());
    HGemPtr->SendLocalData();
}
// ---------------------------------------------------------------------------
// V 1.0
...
void HTGem::S100F4_ReportAllAlarm()
{
    ...
    {
        HGemPtr->DataItemOut(3, HType.LIST_TYPE ,NULL);
        HGemPtr->DataItemOut(8, HType.ASCII_TYPE, HGemPtr->strGrdAlarm->Cells[1][i].c_str());
        HGemPtr->DataItemOut(40, HType.ASCII_TYPE, HGemPtr->strGrdAlarm->Cells[4][i].c_str());
...
void HTGem::S101F2_CurrentEPPDData()
{
    ...
    HGemPtr->DataItemOut(2, HType.LIST_TYPE, NULL);
    HGemPtr->DataItemOut(HGemPtr->SV_71_ASCII_FilenameExtened.Length(), HType.ASCII_TYPE, HGemPtr->SV_71_ASCII_FilenameExtened.c_str());
    HGemPtr->DataItemOut(count, HType.LIST_TYPE, NULL);
...
void HTGem::S101F8_StoreHostUploadFile()
{
    ...
    if(HGemPtr->DataItemIn(5, HType.LIST_TYPE, NULL)==1)
    {
        ...
        if(HGemPtr->GetDataItemLenAndType(len, Type)==1)
        {
            ...
            S=HGemPtr->CurrentDirectory+"\\"+AnsiString(PathName);
            if(!DirectoryExists(S))    //13.09.09.01   klutter
                mkdir(S.c_str());
            S=HGemPtr->CurrentDirectory+"\\"+AnsiString(PathName)+AnsiString("\\")+AnsiString(str);
            if(iStoreCT==1)
                P=fopen(S.c_str(), "wb");
            else
                P=fopen(S.c_str(), "ab+");
