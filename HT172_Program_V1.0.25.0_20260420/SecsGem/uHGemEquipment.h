...
//        int  CheckECValue(AnsiString ECID,void *PtrSour);
        ...
        void __fastcall ManualCreateRadioGroup1Click(TObject *Sender);
...
//##############################################################################
        ...
        TMemoryStream *RecvMemoryBuffer; //test
        TMemoryStream *ProcBuffer;//test
        TMemoryStream *TempProcBuffer;//test
        int iFileCount;

        TCriticalSection *csSFCodeResponse;                                     // 20221111 Joseph (Jason) 新增 TCriticalSection //
        bool SplitStrByTabOnly(AnsiString &source, AnsiString &result);
        AnsiString CustomTrimRightCRLF(const AnsiString &str);
};
//---------------------------------------------------------------------------
