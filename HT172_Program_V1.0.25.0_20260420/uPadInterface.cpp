...
int iReceiceTask=1;
int iSendTask=1;
AnsiString asPadCommLogPath=AnsiString("D:\\HT-172_Log\\PadCommLog");
//---------------------------------------------------------------------------
...
void __fastcall TfPadInterface::FormShow(TObject *Sender)
{
    ...
    SendCommand("t051490000000");                                               //Front Pad 初始化
    SendCommand("t051491000000");                                               //Front Pad 初始化
...
void TfPadInterface::InitialVariable()
{
    ...
    pn_PadInterface_Front->Visible=false;
    sb_PadInterface_Rear->Visible=false;
    ml_PadInterface_Rear->Visible=false;
    pn_PadInterface_Rear->Left=307;
...
bool TfPadInterface::OpenCommPort()
{
    ...
    try
    {
        HANDLE handle = CreateFile(PadComPort.c_str(),                          //KenHsieh 20211222 : Pad與步進馬達為同一Comport
        ...
        if(bPadInterfaceAndTrayStepCommPort)
            dmTrayMotor->comTrayStepMotor->StartComm();
        else
            PadComm->StartComm();                                                   //KenHsieh 20211222 : Pad與步進馬達為同一Comport
...
bool TfPadInterface::CloseCommPort()
{
    try
    {
        if(bPadInterfaceAndTrayStepCommPort)
            ...
            PadComm->StopComm();                                                    //KenHsieh 20211222 : Pad與步進馬達為同一Comport
...
//                              寫資料到檔案
//==============================================================================
void TfPadInterface::WriteDataToFile(char* cFilePath, char* cData)
{
...
void TfPadInterface::RecordCommunication(AnsiString aTitle, AnsiString Command)
{
    ...
    fComPort->MemoAddString(fComPort->memoPanelCom, aTitle, Command);
    GetTimeInfo();
...
{
    CloseCommPort();
    PadComm->CommName="\\\\.\\"+PadComPort;                                     //KenHsieh 20211222 : Pad與步進馬達為同一Comport
    OpenCommPort();
}
//==============================================================================
...
void TfPadInterface::SendCommand(AnsiString sData)
{
    ...
    if(bPadInterfaceAndTrayStepCommPort)
        dmTrayMotor->comTrayStepMotor->WriteCommData(sData.c_str(), iSize);
    else
        fComPort->PadComm->WriteCommData(sData.c_str(), iSize);                     //KenHsieh 20211222 : Pad與步進馬達為同一Comport
    RecordCommunication("[Send]", sData);
}
//==============================================================================
void TfPadInterface::SendCommand(vector<Byte>& Data)
{
    ...
    Buff[iSize]='\r';
    if(bPadInterfaceAndTrayStepCommPort)
        dmTrayMotor->comTrayStepMotor->WriteCommData(Buff, strlen(Buff));
    else
        fComPort->PadComm->WriteCommData(Buff, strlen(Buff));                       //KenHsieh 20211222 : Pad與步進馬達為同一Comport
...
void __fastcall TfPadInterface::DoScanPanelLed(int iAddress, int iKey)
{
    ...
    {
        {
            if(iAddress==0)                                                     //前面板所有按鍵都失效
            {
                if(iKey&PAD_Pause)                                              //除了 PAD_Psuse //KenHsieh 20220401 : 新增Pause
                {
                    ...
                    PadItem[epn_SnRKPause].mlEvent->Value=true;
                    return;
                }
    ...
    for(int i=0; i<CheckPadItem; i++)
    {
        ...
        {
            if(PadItem[i].iData==PAD_SafeLock)
            {
...
//==============================================================================
void __fastcall TfPadInterface::ProcessSendDataNew()                            //丟通訊資料改為固定時間前後面板的 bPadStatus 狀態，若有改變在一次丟整個面板 SW 狀態
{
    int &iTask=iSendTask;
    int iStatue=0x000000;
    AnsiString sData="";                                                        //KenHsieh 20220308 : 新增為三個面板
    static AnsiString sData1="";                                                //KenHsieh 20220308 : 新增為三個面板
//    int iTag=1;                                                                 //循環切換丟前後面板
    ...
    switch(iTask)
    {
        case 1:
            ...
            SendCommand(sData);     //t051490000001
                iTask=10;
            tSendDataDelay.SetMSAndOn(550);                                     //先試試看 300ms
            break;
        case 7:
            if(tSendDataDelay.Off())
            {
                SendCommand(sData1);
                tSendDataDelay.SetMSAndOn(550);                                 //先試試看 300ms
                iTask=10;
...
void __fastcall TfPadInterface::Main232()
{
    ...
    ProcessReceiceData();
    ProcessSendDataNew();
    if(dmTrayMotor->iSetStepMotorTask<6001)                                     //避免頻道互搶
    {
        return;
    }
